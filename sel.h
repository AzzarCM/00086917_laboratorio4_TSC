...
void createLocalC(Matrix &C,mesh m){
    float lambda = (m.getParameter(LAMBDA)*2)/3;

    C.at(0).at(0) += -lambda/2; C.at(0).at(1) += lambda/2;
    C.at(1).at(0) += -lambda/2; C.at(1).at(1) += lambda/2;
...
void createLocalF(Matrix &F,mesh m){
    float alfa = (3*m.getParameter(ALFA));

    F.at(0).at(0) += -alfa/2; F.at(0).at(1) += alfa/2;
    F.at(1).at(0) += -alfa/2; F.at(1).at(1) += alfa/2;
...
void createLocalG(Matrix &G,mesh m){
    ...
    G.at(0).at(0) += -delta/2; G.at(0).at(1) += delta/2;
    G.at(1).at(0) += -delta/2; G.at(1).at(1) += delta/2;
...
Vector createLocalb(int element,mesh &m){
    ...
    float psi = m.getParameter(PSI), l = m.getParameter(LEN), n = m.getParameter(NI);
